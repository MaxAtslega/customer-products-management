...
public class TableOfPanesInCustomerViewer extends Pane {

    private final ArrayList<CustomerSidePane> paneList = new ArrayList<>();
    ...
    public TableOfPanesInCustomerViewer(){
        //set MinSize of two Side Panes With 10 px split
        setMinSize(410,220);
        setStyle("-fx-background-color: transparent");

        for(int i=0; i<2;i++){
            CustomerSidePane pane = new CustomerSidePane(Main.tempStorage.customerList.get(i));
            addPane(pane);
        }

        boundsOfBeginProperty.addListener((observable, oldValue, newValue) -> {
            getAllCustomerInBounds();
    ...
    public void addPane( CustomerSidePane pane) {
        paneList.add(pane);
    ...
    //no failure
    public void getAllCustomerInBounds(){
        ...
        for(int i=boundsOfBegin;i<=boundsOfEnd;i++){
            if(i<Main.tempStorage.customerList.size()){
                addPane(new CustomerSidePane(Main.tempStorage.customerList.get(i)));
            }
        }
    ...
    public String getBoundsOfCustomerIndex(){
        return "B: "+boundsOfBegin+"\nE: "+boundsOfEnd;
    }
}
