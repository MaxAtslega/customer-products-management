...
import dev.atslega.cpmf.TableOfPanesInProductViewer;
...
public class WorkspaceProducts extends Pane {

    private final TableOfPanesInProductViewer table = new TableOfPanesInProductViewer();
    ...
    public WorkspaceProducts () {
        ...
        //Viewer Pane
        Pane productViewer = new Pane();
        productViewer.setPrefWidth(getPrefWidth());
        productViewer.setPrefHeight(getPrefHeight());
        productViewer.setStyle("-fx-background-color: green;");

        // buttomPane
        Pane buttomPane = new Pane();
        buttomPane.setPrefWidth(productViewer.getPrefWidth());
        buttomPane.setPrefHeight(50);
        buttomPane.layoutYProperty().bind(productViewer.heightProperty().subtract(buttomPane.getPrefHeight()));
        ...
        jumpToPane.layoutYProperty().bind(productViewer.heightProperty().subtract(100));
        ...
        ChangeListener<Number> listenerProductPane = new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                double productViewerX = (getWidth());
                double productViewerY = (getHeight());
                productViewer.setPrefWidth(productViewerX);
                productViewer.setPrefHeight(productViewerY);
                buttomPane.setPrefWidth(productViewerX);

                //set table Size
                table.setPrefSize(productViewerX-20,productViewerY-70);
                ...
                table.setBoundsOfProductIndex(sideBoundsBegin.get(sideOnViewer - 1), sideBoundsEnd.get(sideOnViewer - 1));
        ...
        widthProperty().addListener(listenerProductPane);
        heightProperty().addListener(listenerProductPane);
        buttomPane.widthProperty().addListener(listenerButtomPane);
        buttomPane.heightProperty().addListener(listenerButtomPane);

        getChildren().addAll(productViewer,jumpToPane);
        productViewer.getChildren().addAll(table,buttomPane);
    ...
    private void findPageBounds() {
        ...
        int productListSize = Main.tempStorage.productList.size();

        //System.out.println("items per Page: "+itemsPerPage+"\nAll Items: "+customerListSize);

        //set Page if only one page is available
        if (productListSize <= itemsPerPage) {
            sideCount = 1;
            sideBoundsBegin.add(0);
            sideBoundsEnd.add(productListSize-1);
        } else {
            //get Side's
            double a = (double) productListSize / itemsPerPage;
            ...
            for (int i = 0; i < sideCount; i++) {
                ...
                if (end > productListSize - 1) {
                    end = productListSize - 1;
                }
